 ...
 */
public class CSCDR_v5_paper extends Classifier implements OptionHandler, AdditionalMeasureProducer{
...
//        kNN = Math.min(kNN, memoryPool.size());
        ...
        numberOfPrototypes = memoryCells.size();
        if(m_Debug){
            statistics.append(totalGenerations + "," + memoryPool.getLast().getFitness() + "," + memoryPool.getLast().size() +  "\n\n");
            FileWriter fw = new FileWriter(new File(System.getProperty("user.home") + "/cscdr_debug.csv"), true);
            PrintWriter pw = new PrintWriter(fw);
    ...
    public void mutateClone(CSCDRAntibodyPool abPool, Instances aPartition){
        double fitMax = -1;
        ...
        for(int i=0; i < abPool.getAbList().size(); i++){
            ...
            mutateChromosome(abPool.getAbList().get(i), ratio, aPartition);
        }
           
    }
    
    protected void mutateChromosome(Antibody aClone, double aMutationRate, Instances aPartition){
...
//        abPool.setFitness(sumFitness/abPool.size());
    ...
    private final static String [] PARAMETERS = {"C","P","G","S","B","D"};
    ...
    public static void main(String[] argv){
        try{
            System.out.println(Evaluation.evaluateModel(new CSCDR_v5_paper(), argv));
        }
        catch (Exception e){
