...
/**
 * Type: Clonal Selection Classifier with Data Reduction <br>
 * <br>
 * 
 * Description:
 * 
 ...
 */
public class CSCDR extends Classifier implements OptionHandler
{
	// user paramters
    ...
    protected CSCDRAlgorithm algorithm;
...
//        super.setOptions(options);
        ...
        if (aux.length() != 0) {
            setMinimumFitnessThreshold(Double.parseDouble(aux));
        }

        // k-Nearest Neighbours
        aux = Utils.getOption(PARAMETERS[5], options);
        if (aux.length() != 0) {
            setKNN(Integer.parseInt(aux));
        }

        // Total Partitions
        aux = Utils.getOption(PARAMETERS[6], options);
        if (aux.length() != 0) {
            setNumPartitions(Integer.parseInt(aux));
        }
...
//        setNumPartitions(getInteger(PARAMETERS[6], options));
    ...
    public static void main(String[] argv){
        try{
            System.out.println(Evaluation.evaluateModel(new CSCDR(), argv));
        }
        catch (Exception e){
