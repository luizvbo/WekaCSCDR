 ...
 *
 */
public class CSCDR_v4_dissertacao extends Classifier implements OptionHandler{
...
//    	return bestIndex;
    ...
    protected void algorithmPreperation(Instances aAntigens){
        ...
        // pool of antigens used to generate new ab's
        antigenPool = new LinkedList<Integer>();
        // distance metric
...
//        }  
        memoryPool = new LinkedList<CSCDRAntibody>();
        insertRandomAntigens(aAntigens, initialPopulationSize);
    }    
...
// =========================================================================================================================================
            ...
            insertRandomAntigens(data, newAbsPerGeneration);
...
//        kNN = Math.min(kNN, memoryPool.size());
    ...
    // 
    private void initialiseAntigenPool(Instances aPartition){
        for(int j=0; j < aPartition.numInstances(); j++){
            antigenPool.add(j);
        }
        Collections.shuffle(antigenPool);
    }
    
    protected void insertRandomAntigens(Instances aPartition, int quantity){    	
    	// randomise the partition again
//    	aPartition.randomize(rand);
    	// perform insertion
    	for (int i = 0; i < quantity; i++){
            if(antigenPool.isEmpty())
                initialiseAntigenPool(aPartition);
            
            // clone the antigen as an antibody
            CSCDRAntibody clone = new CSCDRAntibody(aPartition.instance(antigenPool.remove(0)));
            // add to pool
...
// =========================================================================================================================================
        ...
        for(int i=0; i < selectedSet.size(); i++){
            ...
            if(selectedSet.get(i).getFitness() > fitMax)
                fitMax = selectedSet.get(i).getFitness();
        }
        for(int i=0; i < selectedSet.size(); i++){
            numClones = (int)Math.round(clonalScaleFactor*selectedSet.size()/(selectedSet.size() - i));
            double D = selectedSet.get(i).getFitness()/fitMax;
            // Parâmetro ro = 3
...
//                range = (range * aMutationRate);
    ...
    public static void main(String[] argv){
        try{
            System.out.println(Evaluation.evaluateModel(new CSCDR_v4_dissertacao(), argv));
        }
        catch (Exception e){
